...
- Reverse geocoding production hardening adds explicit Nominatim `User-Agent`/`Accept` headers, bounded exponential retries for timeout/network/429/5xx failures, request-aware provider logging, a `LOCATION_PROVIDER_ERROR` contract, normalized locality names, and coordinate-keyed TTL caching. Browser analysis now starts independently and uses `Current location` until reverse identity succeeds.
...
Audit results: `pytest -q` has 40 passing tests and one third-party TestClient deprecation warning; `npm run build` succeeds and `git diff --check` is clean. Reverse tests cover validation, successful Nominatim responses, locality extraction, 429/5xx/timeout behavior, cache hits and expiry, the location error contract, and analysis independence. Local end-to-end verification returned HTTP 200 for `/health`, 10 Hyderabad results, `Basar, Telangana, India` for the supplied coordinates, and a fresh Low analysis. Browser visual interaction, device-browser accessibility testing, and automated frontend component tests have not been run. Main limitations: no configured external LLM (by design), practical rather than full NLP validation, reanalysis/grid-point data, three training locations, temporal performance drop, no hydrological data, and no persistent history.
